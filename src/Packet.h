...
struct EmailUnencryptedPacket : public DataPacket{
 public:
  EmailUnencryptedPacket() : DataPacket(DataU) {}
  uint8_t mes_id[32];
  uint8_t DA[32];
  uint16_t fr_id;
  uint16_t fr_count;
...
struct DeletionInfoPacket : public DataPacket{
 public:
  DeletionInfoPacket() : DataPacket(DataT) {}
  struct item {
    uint8_t key[32];
    uint8_t da[32];
    uint32_t time;
...
struct PeerListPacket : public DataPacket{
 public:
  PeerListPacket() : DataPacket(DataL) {}
  uint16_t count;
  std::vector<uint8_t> data;
