...
BoteContext::BoteContext()
    : keys_loaded_(false), listenPortSAM(0), routerPortTCP(0), routerPortUDP(0), bytes_recv_(0), bytes_sent_(0) {
  m_recvQueue = new pbote::util::Queue<std::shared_ptr<PacketForQueue>>;
  m_sendQueue = new pbote::util::Queue<std::shared_ptr<PacketForQueue>>;
  ...
  std::chrono::high_resolution_clock::duration d = std::chrono::high_resolution_clock::now() - std::chrono::high_resolution_clock::now();
...
void BoteContext::send(PacketForQueue packet) {
...
bool BoteContext::receive(std::string sender, pbote::CommunicationPacket packet) {
  ...
  for (auto batch : runningBatches) {
    if (batch->contains(v_cid)) {
      ...
      LogPrint(eLogDebug,"Context: response received for batch ", batch->owner, ", remain response count: ", batch->packetCount() - batch->responseCount());
      return true;
    }
  }
  return false;
...
void BoteContext::init() {
  ...
  if (size > 0) {
    keys_loaded_ = true;
    LogPrint(eLogInfo, "Context: Local identity loaded successfully");
    //LogPrint(eLogDebug, "Context: localDestination=", local_keys_->ToBase64());
  } else {
    keys_loaded_ = false;
    LogPrint(eLogWarning, "Context: Can't find local identity, try to create");
  }
...
int BoteContext::readLocalIdentity(const std::string &path) {
  LogPrint(eLogDebug, "Context: load identity from file ", path);
...
void BoteContext::saveLocalIdentity(const std::string &path) {
  LogPrint(eLogDebug, "Context: save identity to file ", path);
