...
StreamSession::StreamSession(StreamSession &rhs)
    : socket_(rhs.socket_), nickname_(rhs.nickname_),
      sessionID_(generateSessionID()), myDestination_(rhs.myDestination_),
      i2pOptions_(rhs.i2pOptions_), isSick_(false) {
  rhs.fallSick();
  rhs.socket_.close();
...
RequestResult<std::unique_ptr<I2pSocket>> StreamSession::accept(bool silent) {
  ...
  switch (status) {
    case Message::OK:return ResultType(std::move(streamSocket));
    case Message::EMPTY_ANSWER:
    case Message::CLOSED_SOCKET:
    case Message::INVALID_ID:
    case Message::I2P_ERROR:fallSick();
      break;
    default:break;
  }
  return ResultType();
...
RequestResult<void> StreamSession::forward(const std::string &host, uint16_t port, bool silent) {
  ...
  switch (status) {
    case Message::OK:forwardedStreams_.push_back(ForwardedStream(newSocket.get(), host, port, silent));
...
RequestResult<const std::string> StreamSession::namingLookup(const std::string &name) const {
  ...
  switch (answer.status) {
    case Message::OK:return ResultType(answer.value);
    case Message::EMPTY_ANSWER:
    case Message::CLOSED_SOCKET:fallSick();
      break;
    default:break;
  }
  return ResultType();
...
FullDestination StreamSession::createStreamSession(const std::string &destination) {
  ...
  const AnswerType
      answer = createStreamSession(socket_, sessionID_, nickname_, destination, i2pOptions_, SAM_SIGNATURE_TYPE);
  if (answer.status != Message::OK) {
    fallSick();
    return FullDestination();
  }
  return FullDestination(answer.value.substr(0, I2P_DESTINATION_SIZE),
                         answer.value,
                         (destination == SAM_GENERATE_MY_DESTINATION));
}

void StreamSession::fallSick() const { isSick_ = true; }
...
/*static*/
Message::Answer<const std::string> StreamSession::request(I2pSocket &socket,
...
const std::string &StreamSession::getSAMMinVer() const { return socket_.minVer_; }

const std::string &StreamSession::getSAMMaxVer() const { return socket_.maxVer_; }
...
DatagramSession::DatagramSession(
    ...
    const std::string &i2pOptions /*= SAM_DEFAULT_I2P_OPTIONS*/,
    const std::string &signatureType /*= SAM_SIGNATURE_TYPE*/)
    : socket_(SAMHost, SAMPortTCP), nickname_(nickname),
      sessionID_(generateSessionID()), i2pOptions_(i2pOptions), isSick_(false) {
...
/*static*/
std::string DatagramSession::generateSessionID() {
  static const int minSessionIDLength = 5;
  static const int maxSessionIDLength = 9;
  static const char sessionIDAlphabet[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  int length = minSessionIDLength - 1;
...
RequestResult<const std::string> DatagramSession::namingLookup(const std::string &name) const {
  typedef RequestResult<const std::string> ResultType;
  typedef Message::Answer<const std::string> AnswerType;
...
RequestResult<const FullDestination> DatagramSession::destGenerate() const {
  typedef RequestResult<const FullDestination> ResultType;
  typedef Message::Answer<const FullDestination> AnswerType;
...
FullDestination DatagramSession::createDatagramSession(const std::string &destination) {
  ...
  const AnswerType answer = createDatagramSession(socket_,
                                                  sessionID_,
                                                  nickname_,
                                                  listenPortUDP_,
                                                  listenAddress_,
...
void DatagramSession::fallSick() const { isSick_ = true; }

/*static*/
Message::Answer<const std::string> DatagramSession::rawRequest(I2pSocket &socket, const std::string &requestStr) {
  typedef Message::Answer<const std::string> AnswerType;
...
/*static*/
Message::Answer<const std::string> DatagramSession::request(I2pSocket &socket,
                                                            const std::string &requestStr,
                                                            const std::string &keyOnSuccess) {
  typedef Message::Answer<const std::string> AnswerType;
...
/*static*/
Message::eStatus DatagramSession::request(I2pSocket &socket, const std::string &requestStr) {
  return rawRequest(socket, requestStr).status;
}
...
/*static*/
Message::Answer<const std::string> DatagramSession::namingLookup(I2pSocket &socket, const std::string &name) {
  return request(socket, Message::namingLookup(name), "VALUE");
}

/*static*/
Message::Answer<const FullDestination> DatagramSession::destGenerate(I2pSocket &socket) {
  // while answer for a DEST GENERATE request doesn't contain a "RESULT" field we parse it manually
  typedef Message::Answer<const FullDestination> ResultType;
...
const std::string &DatagramSession::getNickname() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getNickname: " << nickname_ << std::endl;
#endif // DEBUG_ON_STDOUT
  return nickname_;
...
const std::string &DatagramSession::getSessionID() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getSessionID: " << sessionID_ << std::endl;
#endif // DEBUG_ON_STDOUT
  return sessionID_;
...
const std::string &DatagramSession::getOptions() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getOptions: " << i2pOptions_ << std::endl;
#endif // DEBUG_ON_STDOUT
  return i2pOptions_;
...
const FullDestination &DatagramSession::getMyDestination() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getMyDestination: " << myDestination_.priv << std::endl;
#endif // DEBUG_ON_STDOUT
  return myDestination_;
...
bool DatagramSession::isSick() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "isSick: " << isSick_ << std::endl;
#endif // DEBUG_ON_STDOUT
  return isSick_;
...
const sockaddr_in &DatagramSession::getSAMAddress() const {
#ifdef DEBUG_ON_STDOUT
  // std::cout << "getSAMAddress: " << socket_.getAddress() << std::endl;
    std::cout << "getSAMAddress: not yet parsed" << std::endl;
#endif // DEBUG_ON_STDOUT
...
const std::string &DatagramSession::getSAMHost() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getSAMHost: " << socket_.getHost() << std::endl;
#endif // DEBUG_ON_STDOUT
  return socket_.getHost();
...
uint16_t DatagramSession::getSAMPort() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getSAMPort: " << socket_.getPort() << std::endl;
#endif // DEBUG_ON_STDOUT
  return socket_.getPort();
...
const std::string &DatagramSession::getSAMVersion() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getSAMVersion: " << socket_.getVersion() << std::endl;
#endif // DEBUG_ON_STDOUT
  return socket_.getVersion();
...
const std::string &DatagramSession::getSAMMinVer() const { return socket_.minVer_; }

const std::string &DatagramSession::getSAMMaxVer() const { return socket_.maxVer_; }
...
/*static*/
std::string RawSession::generateSessionID() {
  static const int minSessionIDLength = 5;
  static const int maxSessionIDLength = 9;
  static const char sessionIDAlphabet[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  int length = minSessionIDLength - 1;
...
RequestResult<const std::string> RawSession::namingLookup(const std::string &name) const {
  typedef RequestResult<const std::string> ResultType;
  typedef Message::Answer<const std::string> AnswerType;
...
RequestResult<const FullDestination> RawSession::destGenerate() const {
  typedef RequestResult<const FullDestination> ResultType;
  typedef Message::Answer<const FullDestination> AnswerType;
...
FullDestination RawSession::createRawSession(const std::string &destination) {
  ...
  const AnswerType answer = createRawSession(socket_,
                                             sessionID_,
                                             nickname_,
                                             listenPortUDP_,
                                             listenAddress_,
...
void RawSession::fallSick() const { isSick_ = true; }

/*static*/
Message::Answer<const std::string> RawSession::rawRequest(I2pSocket &socket, const std::string &requestStr) {
  typedef Message::Answer<const std::string> AnswerType;
...
/*static*/
Message::Answer<const std::string> RawSession::request(I2pSocket &socket,
                                                       const std::string &requestStr,
                                                       const std::string &keyOnSuccess) {
  typedef Message::Answer<const std::string> AnswerType;
...
/*static*/
Message::eStatus RawSession::request(I2pSocket &socket, const std::string &requestStr) {
  return rawRequest(socket, requestStr).status;
}
...
Message::Answer<const std::string> RawSession::createRawSession(I2pSocket &socket,
                                                                ...
                                                                const std::string &signatureType) {
  return request(socket,
                 Message::sessionCreate(Message::sssRaw,
                                        sessionID,
                                        nickname,
                                        port,
                                        host,
...
/*static*/
Message::Answer<const std::string> RawSession::namingLookup(I2pSocket &socket, const std::string &name) {
  return request(socket, Message::namingLookup(name), "VALUE");
}

/*static*/
Message::Answer<const FullDestination> RawSession::destGenerate(I2pSocket &socket) {
  // while answer for a DEST GENERATE request doesn't contain a "RESULT" field we parse it manually
  typedef Message::Answer<const FullDestination> ResultType;
...
const std::string &RawSession::getNickname() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getNickname: " << nickname_ << std::endl;
#endif // DEBUG_ON_STDOUT
  return nickname_;
...
const std::string &RawSession::getSessionID() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getSessionID: " << sessionID_ << std::endl;
#endif // DEBUG_ON_STDOUT
  return sessionID_;
...
const std::string &RawSession::getOptions() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getOptions: " << i2pOptions_ << std::endl;
#endif // DEBUG_ON_STDOUT
  return i2pOptions_;
...
const FullDestination &RawSession::getMyDestination() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getMyDestination: " << myDestination_.priv << std::endl;
#endif // DEBUG_ON_STDOUT
  return myDestination_;
...
const sockaddr_in &RawSession::getSAMAddress() const {
#ifdef DEBUG_ON_STDOUT
  // std::cout << "getSAMAddress: " << socket_.getAddress() << std::endl;
    std::cout << "getSAMAddress: not yet parsed" << std::endl;
#endif // DEBUG_ON_STDOUT
...
const std::string &RawSession::getSAMHost() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getSAMHost: " << socket_.getHost() << std::endl;
#endif // DEBUG_ON_STDOUT
  return socket_.getHost();
...
uint16_t RawSession::getSAMPort() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getSAMPort: " << socket_.getPort() << std::endl;
#endif // DEBUG_ON_STDOUT
  return socket_.getPort();
...
const std::string &RawSession::getSAMVersion() const {
#ifdef DEBUG_ON_STDOUT
  std::cout << "getSAMVersion: " << socket_.getVersion() << std::endl;
#endif // DEBUG_ON_STDOUT
  return socket_.getVersion();
...
const std::string &RawSession::getSAMMinVer() const { return socket_.minVer_; }
