...
void RelayPeersWorker::run() {
  ...
  while (started_) {
    ...
    batch->owner = "RelayPeersWorker::main";
    ...
    for (auto response : responses) {
      ...
      if (packetReceived(data, dataLen)) {
...
bool RelayPeersWorker::addPeer(const uint8_t *buf, int len) {
  ...
  if (identity.FromBuffer(buf, len))
    return addPeer(identity.GetIdentHash(), buf, len);
  return false;
}

bool RelayPeersWorker::addPeer(const i2p::data::IdentHash &ident, const uint8_t *buf, int len) {
  if (findPeer(ident))
    return false;

  auto peer = std::make_shared<RelayPeer>(buf, len);
...
bool RelayPeersWorker::packetReceived(const unsigned char *buf, size_t len) {
  size_t offset = 0;
  uint8_t type, ver;
  uint16_t nump;
  ...
  if ((type == (uint8_t) 'L' || type == (uint8_t) 'P') && ver == (uint8_t) 4) {
    ...
    for (size_t i = 0; i < nump; i++) {
      if (offset == len) {
        LogPrint(eLogWarning, "RelayPeers: packetReceived: end of packet!");
        break;
      }
      if (offset + 384 > len) {
        LogPrint(eLogWarning, "RelayPeers: packetReceived: incomplete packet!");
        break;
      }
...
void RelayPeersWorker::peerListRequestV4(const std::string &sender, const uint8_t *cid) {
  ...
  pbote::PeerListPacket newResult;
