...
void UDPReceiver::setQueue(pbote::util::Queue<std::shared_ptr<PacketForQueue>> *recvQueue) { m_recvQueue = recvQueue; }

int UDPReceiver::get_socket() const { return f_socket; }

int UDPReceiver::get_port() const { return f_port; }

std::string UDPReceiver::get_addr() const { return f_addr; }

bool UDPReceiver::isRunning() { return m_IsRunning; }
...
int UDPReceiver::recv() { return ::recv(f_socket, m_DatagramRecvBuffer, MAX_DATAGRAM_SIZE, 0); }
...
void UDPSender::setQueue(pbote::util::Queue<std::shared_ptr<PacketForQueue>> *sendQueue) { m_sendQueue = sendQueue; }

int UDPSender::get_socket() const { return f_socket; }

int UDPSender::get_port() const { return f_port; }

std::string UDPSender::get_addr() const { return f_addr; }

bool UDPSender::isRunning() { return m_IsRunning; }
...
void UDPSender::send(/*const char *msg, size_t size*/) {
  ...
  bytes_transferred = sendto(f_socket, message.c_str(), message.size(), 0, f_addrinfo->ai_addr, f_addrinfo->ai_addrlen);
  if (m_IsRunning) {
...
void NetworkWorker::start() {
  LogPrint(eLogInfo, "Network: starting SAM session with ", routerAddress_, ":", routerPortTCP_, " | ",
           routerAddress_, ":", routerPortUDP_);
  try {
    auto key = createSAMSession();
    if (!context.keys_loaded())
      context.save_new_keys(key);

    // need here, because we get sessionID after SAM initialization
