...
void EmailWorker::start() {
  if (!started_) {
    ...
    if (email_identities.empty()) {
      LogPrint(eLogError, "EmailWorker: have no identities for search!");
    } else {
      LogPrint(eLogInfo, "EmailWorker: have ", email_identities.size(), " identities for search");
      startCheckEmailTask();
      startSendEmailTask();
    }
...
pbote::IndexPacket EmailWorker::parseIndexPkt(uint8_t *buf, size_t len, bool from_net) {
  ...
  if (len < 38) {
    LogPrint(eLogWarning, "Email: parseIndexPkt: payload is too short");
    return {};
  }
  IndexPacket packet;
  uint16_t offset = 0;
  ...
  LogPrint(eLogDebug, "Email: parseIndexPkt: nump: ", packet.nump);

  if (!(packet.type == (uint8_t) 'I')) {
    LogPrint(eLogWarning, "Email: parseIndexPkt: wrong packet type: ", packet.type);
    return {};
  }

  if (!(packet.ver == (uint8_t) 4)) {
    LogPrint(eLogWarning, "Email: parseIndexPkt: wrong packet version: ", (unsigned) packet.ver);
    return {};
  }

  // Check if payload length enough to parse all entries
  if (len < (38 + (68 * packet.nump))) {
    LogPrint(eLogWarning, "Email: parseIndexPkt: incomplete packet!");
    return {};
  }
  ...
  for (uint32_t i = 0; i < packet.nump; i--) {
    ...
    LogPrint(eLogDebug, "Email: parseIndexPkt: mail key: ", key.ToBase64());

    std::memcpy(&entry.dv, buf + offset, 32);
    offset += 32;
    i2p::data::Tag<32> dv(entry.dv);
    LogPrint(eLogDebug, "Email: parseIndexPkt: mail dvr: ", dv.ToBase64());
...
pbote::EmailEncryptedPacket EmailWorker::parseEmailEncryptedPkt(uint8_t *buf, size_t len, bool from_net) {
  LogPrint(eLogDebug, "Email: parseEmailEncryptedPkt: payload size: ", len);
  /// 105 cause type[1] + ver[1] + key[32] + stored_time[4] + delete_hash[32] + alg[1] + length[2] + DA[32]
  if (len < 105) {
    LogPrint(eLogWarning, "Email: parseEmailEncryptedPkt: payload is too short");
    return {};
  }
  ...
  if (!(packet.type == (uint8_t) 'E')) {
    LogPrint(eLogWarning, "Email: parseEmailEncryptedPkt: wrong packet type: ", packet.type);
    return {};
  }

  if (!(packet.ver == (uint8_t) 4)) {
    LogPrint(eLogWarning, "Email: parseEmailEncryptedPkt: wrong packet version: ", (unsigned) packet.ver);
    return {};
  }
  ...
  LogPrint(eLogDebug, "Email: parseEmailEncryptedPkt: packet.stored_time: ", packet.stored_time);
  offset += 4;
  std::memcpy(&packet.delete_hash, buf + offset, 32);
  offset += 32;
  std::memcpy(&packet.alg, buf + offset, 1);
  LogPrint(eLogDebug, "Email: parseEmailEncryptedPkt: packet.alg: ", packet.alg);
  offset += 1;

  std::vector<uint8_t> data_for_verify(buf + offset, buf + len);

  std::memcpy(&packet.length, buf + offset, 2);
  LogPrint(eLogDebug, "Email: parseEmailEncryptedPkt: packet.length: ", packet.length);
  ...
  LogPrint(eLogDebug, "Email: parseEmailEncryptedPkt: ver_hash: ", ver_hash.ToBase64());
  LogPrint(eLogDebug, "Email: parseEmailEncryptedPkt: cur_hash: ", cur_hash.ToBase64());
  //i2p::data::Tag<32> del_hash(packet.delete_hash);
  //LogPrint(eLogDebug, "Email: parseEmailEncryptedPkt: del_hash: ", del_hash.ToBase64());
  if (ver_hash != cur_hash) {
    LogPrint(eLogError, "Email: parseEmailEncryptedPkt: hash mismatch");
    return {};
  }
  ...
  LogPrint(eLogDebug, "Email: parseEmailEncryptedPkt: alg: ", (unsigned) packet.alg, ", length: ", packet.length);
...
void EmailWorker::run() {
  while (started_) {
    ...
    if (!m_check_email_thread_ && started_) {
      ...
      if (!email_identities.empty()) {
        LogPrint(eLogDebug, "EmailWorker: have ", email_identities.size(), " identities, start startCheckEmailTask");
        startCheckEmailTask();
      }
    }
    ...
    if (!m_send_email_thread_ && started_) {
      ...
      if (!email_identities.empty()) {
        LogPrint(eLogDebug, "EmailWorker: have ", email_identities.size(), " identities, start startSendEmailTask");
        startSendEmailTask();
      }
    }
...
void EmailWorker::checkEmailTask() {
  while (started_) {
    for (const auto &email_identity : email_identities) {
      // ToDo: merge local and retrieved packets
      //auto index_packets = retrieveIndex(email_identity);
      //auto index_packets = loadLocalIndex();
      //LogPrint(eLogDebug, "Email: checkEmailTask: index count: ", index_packets.size());

      // ToDo: merge local and retrieved packets
      //auto enc_mail_packets = retrieveEmailEncryptedPacket(index_packets);
      auto enc_mail_packets = loadLocalEmailEncryptedPacket();
      LogPrint(eLogDebug, "Email: checkEmailTask: mail count: ", enc_mail_packets.size());
    ...
    LogPrint(eLogInfo, "Email: checkEmailTask: Round complete");
    std::this_thread::sleep_for(std::chrono::seconds(60));
  }
}
...
std::vector<pbote::EmailEncryptedPacket> EmailWorker::retrieveEmailEncryptedPacket(const std::vector<pbote::IndexPacket> &index_packets) {
  ...
  LogPrint(eLogDebug, "EmailWorker: retrieveEmailEncryptedPacket: ", mail_packets.size(), " mail packets parsed");
...
std::vector<std::shared_ptr<pbote::Email>> EmailWorker::checkOutbox() {
  ...
  LogPrint(eLogDebug, "EmailWorker: checkOutbox: found ", emails.size(), " emails for send.");
...
/*bool EmailWorker::saveEmailUnencryptedPacket(pbote::EmailUnencryptedPacket packet) {
  i2p::data::Tag<32> mes_id(packet.mes_id);
  // <MS_ID>_<FR_ID>.pkt
  std::string packetPath = pbote::fs::DataDirPath("incomplete", mes_id.ToBase64() + "_" + std::to_string(packet.fr_id) + ".pkt");

  if (pbote::fs::Exists(packetPath)) {
    return false;
  }
  LogPrint(eLogDebug, "Email: saveEmailUnencryptedPacket: save packet to ", packetPath);
  std::ofstream file(packetPath, std::ofstream::binary | std::ofstream::out);
  if (!file.is_open()) {
    LogPrint(eLogError, "Email: saveEmailUnencryptedPacket: can't open file ", packetPath);
    return false;
  }

  file.write(reinterpret_cast<const char *>(&packet.type), sizeof(packet.type));
  file.write(reinterpret_cast<const char *>(&packet.ver), sizeof(packet.ver));
  file.write(reinterpret_cast<const char *>(&packet.mes_id), sizeof(packet.mes_id));
  file.write(reinterpret_cast<const char *>(&packet.DA), sizeof(packet.DA));
  file.write(reinterpret_cast<const char *>(&packet.fr_id), sizeof(packet.fr_id));
  file.write(reinterpret_cast<const char *>(&packet.fr_count), sizeof(packet.fr_count));
  file.write(reinterpret_cast<const char *>(&packet.length), sizeof(packet.length));
