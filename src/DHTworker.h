...
class DHTworker {
 public:
  ...
  bool addNode(const i2p::data::IdentHash &ident, const uint8_t *buf, size_t len);
  std::shared_ptr<Node> findNode(const i2p::data::IdentHash &ident) const; /// duplication check

  //std::shared_ptr<Peer> getClosestNode(const i2p::data::IdentHash & destination, bool to_us);
  //std::vector<Peer> getClosestNodes(i2p::data::IdentHash key, size_t num, bool to_us);
  ...
  std::vector<Node> packetReceived(const unsigned char* buf, size_t len);

 private:
  void run();

  std::vector<std::string> readNodes();
  bool loadNodes();
  void writeNodes();
  //bool startBootstrap();
